...
def naive_convolve(fircoeffs, wavdata):
    ...
    return [sum(a * b
            ...
            for i in xrange(len(wavdata) + len(fircoeffs) - 1)]
...
def load_wave_as_mono_s16(filename):
    little = array.array('H', '\x01\x00')[0] == 1
...
##              % (filterid, 1 << logquant, repr(resids), repr(list(piece))))
        out.append(byte0)
        for i in xrange(0, len(resids), 2):
...
def decode_brr(brrdata):
    prevs = [0, 0]
    out = []
    for i in xrange(0, len(brrdata), 9):
...
def parse_argv(argv):
    ...
    parser.add_option("-r", "--rate", dest="rate",
                      ...
                      help="set the wave sample rate in Hz (default: 8372)")
    ...
    pos = iter(pos)
    try:
        options.infilename = options.infilename or pos.next()
    except StopIteration:
        parser.error("no input file; try %s --help"
                     % os.path.basename(sys.argv[0]))
    try:
        options.outfilename = options.outfilename or pos.next()
    except StopIteration:
